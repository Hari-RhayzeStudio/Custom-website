...
function DesignResultContent() {
  ...
  const handleGenerateEdit = async () => {
    ...
    try {
      ...
      formData.append('image', new File([await imageRes.blob()], "current.png"));
      formData.append('x', hotspot.x.toString()); formData.append('y', hotspot.y.toString());
    ...
    <div className="min-h-screen bg-white font-sans pb-20">
      <div className="max-w-6xl mx-auto px-6 py-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
              <Link href="/design" className="p-2 hover:bg-gray-100 rounded-full transition text-gray-600"><ArrowLeft className="w-5 h-5" /></Link>
              <h1 className="text-lg text-gray-700 font-medium truncate max-w-500px capitalize">{data.prompt}</h1>
          </div>
          {history.length > 1 && !isEditing && <button onClick={handleUndo} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200"><Undo2 className="w-4 h-4" /> Undo</button>}
      </div>

      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <DesignImagePreview imageUrl={data.imageUrl} isEditing={isEditing} setIsEditing={setIsEditing} hotspot={hotspot} onImageClick={handleImageClick} onSave={handleSave} isSaving={isSaving} imageRef={imageRef} />
...
export default function DesignResultPage() {
  return <Suspense fallback={<DesignGenerationLoader />}><DesignResultContent /></Suspense>;
}