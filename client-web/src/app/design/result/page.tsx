...
function DesignResultContent() {
  ...
  // ✅ FIXED: Generate edit function with URL update
  const handleGenerateEdit = async () => {
    ...
    console.log(`🎨 Generating edit: "${editPrompt}" at hotspot (${hotspot.x}, ${hotspot.y})`);
    ...
    try {
      ...
      formData.append('image', new File([blob], "current.png", { type: blob.type }));
      ...
      // Call edit API
      const res = await fetch(`${API_BASE_URL}/api/edit-design`, { 
        method: 'POST', 
        body: formData 
      });
      ...
      if (result.imageUrl) {
        ...
        // ✅ UPDATE URL to reflect new prompt
        const cleanPrompt = editPrompt.replace(/[^a-z0-9]/gi, '-').toLowerCase();
        router.push(`/design/result?prompt=${cleanPrompt}`, { scroll: false });
        
        // ✅ Reset edit state
        setEditPrompt("");
        setHotspot({x:0, y:0});
        
        console.log('✅ Edit successful, new image loaded, URL updated');
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/wishlist`, {
        ...
        body: JSON.stringify({ 
          user_id: userId, 
          product_sku: `gen-${Date.now()}`, 
          product_name: `Custom: ${data?.prompt.substring(0, 30)}`, 
        ...
        if (req.status === 'pending') {
          ...
          try {
            ...
            if (req.base64Image) {
               ...
               if(req.hotspot) { 
                 formData.append('x', req.hotspot.x.toString()); 
                 formData.append('y', req.hotspot.y.toString()); 
               }
  ...
  const handleUndo = () => {
    ...
    // ✅ Update URL to match the previous prompt
    const cleanPrompt = prev.prompt.replace(/[^a-z0-9]/gi, '-').toLowerCase();
    router.push(`/design/result?prompt=${cleanPrompt}`, { scroll: false });
  };
  ...
  if (!data) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="text-xl text-gray-600 mb-4">No design found</p>
        <Link href="/design">
    ...
    <div className="min-h-screen bg-white font-sans pb-20">
      <div className="max-w-6xl mx-auto px-6 py-8 flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
              <Link href="/design" className="p-2 hover:bg-gray-100 rounded-full transition text-gray-600 shrink-0">
          ...
          {history.length > 1 && !isEditing && (
            <button 
              onClick={handleUndo} 
              className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-gray-600 bg-gray-100 rounded-full hover:bg-gray-200 shrink-0"
            >
          ...
          {isEditing ? (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
               ...
               <div className="bg-[#FAF8F3] p-6 rounded-2xl border border-[#F0E6D2] mb-6 text-sm text-gray-600">
                 <p>1. Click on the image to place the edit point (optional).</p>
                 <p>2. Describe your change below.</p>
                 {hotspot.x > 0 && (
                   <p className="mt-2 text-purple-600 font-medium">
                     ✓ Edit point set at ({hotspot.x}, {hotspot.y})
                   </p>
                 )}
               </div>
               ...
               <textarea 
                 ...
                 placeholder="Describe what you want to change (e.g., 'make this yellow', 'add diamonds', 'change to gold')" 
